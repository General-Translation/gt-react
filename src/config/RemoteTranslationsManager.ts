 ...
 */
export function getDictionaryReference(locale: string, dictionaryName: string): string {
...
export class RemoteTranslationsManager {
     ...
     */
    async getTranslations(locale: string, dictionaryName: string): Promise<Record<string, any> | null> {
     ...
     */
    setTranslations(locale: string, dictionaryName: string, key: string, id: string = key, translation: any): boolean {
        if (!(locale && dictionaryName && key && id && translation)) return false;
        const reference = getDictionaryReference(locale, dictionaryName);
        const currentDictionary = this.dictionaryMap.get(reference) || {};
        this.dictionaryMap.set(reference, { ...currentDictionary, [id]: 
            (translation && typeof translation === 'object' && translation.t) ? { ...translation, k: key } : { k: key, t: translation }
        });
        return true;
    }
...
const remoteDictionaryManager = new RemoteTranslationsManager();
